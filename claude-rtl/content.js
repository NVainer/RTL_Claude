(() => {
  ...
  // Does this block contain any Hebrew/Arabic at all? If so, it's a candidate.
  function hasRtl(text) {
    return (text.match(RTL) || []).length > 0;
  }

  // We mark candidate blocks with dir="auto" and let the browser decide direction
  // from the FIRST strong character of the paragraph. This beats a majority-of-chars
  // rule: a Hebrew sentence stays RTL even when it's packed with English tech terms.
  function tag(el) {
    if (!el || el.nodeType !== 1) return;
    if (el.closest("pre, code")) return; // never touch code
    if (!hasRtl(el.textContent || "")) return; // skip pure-LTR blocks entirely
    ...
    if (list && list.getAttribute("dir") !== "auto") {
